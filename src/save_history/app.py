...
def save_history(event):
    ...
    for field in required_fields:
        if field not in body:
            return {
                ...
                "body": json.dumps(
                    {"error": f"{field}は必須項目です"}, ensure_ascii=False
                ),
            }
        ...
        if meal_type in body["meals"]:
            ...
            for recipe in meal_data:
                if (
                    ...
                    or "recipe_id" not in recipe
                    or "name" not in recipe
                ):
                    return {
                        "statusCode": 400,
                        "headers": {
    ...
    return {
        ...
        "body": json.dumps(
            {"message": "献立履歴を保存しました", "history": decimal_to_float(history)},
            ensure_ascii=False,
        ),
    }
...
def get_history(event):
    ...
    return {
        ...
        "body": json.dumps(
            {"history": history_list, "count": len(history_list)}, ensure_ascii=False
        ),
    }
