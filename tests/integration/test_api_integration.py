...
class TestRecipeWorkflow:
    ...
    def test_create_and_get_recipe_workflow(self, lambda_context, mock_dynamodb_recipes_table):
    ...
    def test_create_multiple_recipes_and_filter(self, lambda_context, mock_dynamodb_recipes_table):
...
class TestHistoryWorkflow:
    ...
    def test_save_and_get_history_workflow(self, lambda_context, mock_dynamodb_history_table):
        ...
        save_event = {
            ...
            'body': json.dumps({
                ...
                'meals': {
                    ...
                    'lunch': [
                        {'recipe_id': 'recipe_002', 'name': 'ハンバーグ'}
                    ],
                    'dinner': [
                        {'recipe_id': 'recipe_003', 'name': '鮭の塩焼き'}
                    ]
                },
                'notes': '統合テスト'
            })
...
class TestCompleteMenuPlanningWorkflow:
    ...
    @mock_dynamodb
    def test_full_workflow(self, lambda_context, mock_dynamodb_recipes_table, mock_dynamodb_history_table):
        ...
        save_event = {
            ...
            'body': json.dumps({
                ...
                'meals': {
                    'breakfast': [
                        {'recipe_id': created_recipes[0]['recipe_id'], 'name': created_recipes[0]['name']}
                    ],
                    'lunch': [
                        {'recipe_id': created_recipes[2]['recipe_id'], 'name': created_recipes[2]['name']}
                    ],
                    'dinner': [
                        {'recipe_id': created_recipes[3]['recipe_id'], 'name': created_recipes[3]['name']}
                    ]
                }
            })
        }
